...
def above_threshold(student_scores, threshold):
    ...
    return list(filter(lambda s: s >= threshold, student_scores))
...
def student_ranking(student_scores, student_names):
    ...
    return ["{}. {}: {}".format(*t)
            for t in zip(range(1, len(student_names)+1), student_names, student_scores)]
...
def perfect_score(student_info):
    ...
    for name_score in student_info:
        match name_score:
            case [_, 100] as found: return found
            case _: pass
